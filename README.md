...
1. Enter your **stock tickers** (comma-separated, e.g. `AAPL,MSFT,NVDA`).
...
4. Edit the **Positions** table with your actual dollar NMVs (defaults to
   an equal-weighted $10 long in each ticker so you see a result immediately).
5. Click **Fetch / Refresh data**.

Data is cached for 1 hour per ticker/date-range combination, so tweaking
other settings (factor groupings, min-observations slider) afterward
won't re-hit the network. Click the fetch button again any time you want
fresh prices.
